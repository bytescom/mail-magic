...
const IntegrationsTab = () => {
    const [gmailEmail, setGmailEmail] = useState("pankaj@gmail.com");
                ...
                <div className="flex flex-col gap-6 w-full lg:w-3/4">
                    <div className="flex items-center justify-between px-5 py-4 bg-background border border-border rounded-xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h4 className="text-[14px] font-medium text-text-dark">Gmail Account</h4>
                                {gmailEmail ? (
                                    <p className="text-[13px] text-emerald-600 flex items-center gap-1 font-medium">
                        ...
                        {gmailEmail ? (
                            <button
                                onClick={() => setGmailEmail(null)}
                                className="flex items-center gap-1.5 px-4 py-2 border border-red-200 bg-red-50 text-red-500 rounded-md text-[13px] font-medium hover:bg-red-100 transition-colors cursor-pointer"
                            >
                                <FiX size={13} /> Disconnect
                            </button>
                        ) : (
                            <button
                                onClick={() => setGmailEmail("pankaj@gmail.com")}
                                className="px-4 py-2 bg-primary text-white rounded-md text-[13px] font-medium hover:opacity-90 transition-colors cursor-pointer"
                            >
                                Connect Gmail
                            </button>
                        )}
                    ...
                    {!gmailEmail && (
                        <div className="p-4 bg-surface border border-border rounded-lg flex items-start gap-3">
                            <FiAlertTriangle className="text-muted mt-0.5 shrink-0" size={15} />
                            <p className="text-[13px] text-text leading-relaxed">
                                We use OAuth to securely connect to your Gmail. We never store passwords and only request permission to <strong>send emails</strong> on your behalf.
                            </p>
                        </div>
                    )}
                    ...
                    {gmailEmail && (
                        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg flex items-start gap-3">
                            ...
                            <p className="text-[13px] text-emerald-700 leading-relaxed font-medium">
                                Gmail connected. Emails generated on the Outreach page will be sent from <strong>{gmailEmail}</strong>.
                            </p>
                        </div>
                    )}
                </div>
