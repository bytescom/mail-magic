...
export class GmailService {
    ...
    async refreshAccessToken() {
        try {
            ...
            if (!currentCredentials.refresh_token) {
                ...
                throw new Error('MISSING_REFRESH_TOKEN: No refresh token available. User must re-authenticate.');
            }

            console.log('🔑 Attempting to refresh with refresh token...');
            ...
            console.log('📝 New token expiry:', credentials.expiry_date ? new Date(credentials.expiry_date) : 'unknown');
            ...
            if (this.userId && credentials.access_token) {
                try {
                    ...
                    // Update refresh token if provided (Google sometimes rotates it)
                    if (credentials.refresh_token) {
                        updateData.refreshToken = credentials.refresh_token;
                        console.log('🔄 Refresh token was rotated by Google');
                    }
            ...
            if (error.message?.includes('invalid_grant')) {
                throw new Error('INVALID_GRANT: Refresh token is invalid or revoked. User must sign out and sign in again.');
            } else if (error.message?.includes('MISSING_REFRESH_TOKEN')) {
    ...
    /**
     * Encode subject line according to RFC 2047 for proper handling of special characters
     * This prevents encoding issues with apostrophes, dashes, emojis, and non-ASCII characters
     */
    encodeSubject(subject) {
        // Check if subject contains non-ASCII characters
        if (/^[\x00-\x7F]*$/.test(subject)) {
            // Subject contains only ASCII characters, no encoding needed
            return subject;
        }

        // Encode using RFC 2047 MIME encoding (Base64)
    ...
    /**
     * Encode filename according to RFC 2047 for proper handling of special characters
     */
    encodeFilename(filename) {
        // Check if filename contains non-ASCII characters
        if (/^[\x00-\x7F]*$/.test(filename)) {
            // Filename contains only ASCII characters, no encoding needed
            return filename;
        }
    ...
    createMessageWithAttachments({ to, subject, body, from, attachments, CRLF = '\r\n' }) {
        ...
        for (const attachment of attachments) {
            const encodedFilename = this.encodeFilename(attachment.filename);

            messageParts.push(`--${boundary}`);
            messageParts.push(`Content-Type: ${attachment.mimeType}; name="${encodedFilename}"`);
            messageParts.push('Content-Transfer-Encoding: base64');
            messageParts.push(`Content-Disposition: attachment; filename="${encodedFilename}"`);
            messageParts.push('');
    ...
    /**
     * Send an email as a reply within an existing Gmail thread.
     * Used by the follow-up cron job to stay in the same conversation.
     */
    async sendEmailInThread({ threadId, to, subject, body, from }) {
        try {
            const CRLF = '\r\n';
            const formattedBody = `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.7; color: #222222; white-space: pre-wrap;">${body}</div>`;

            const message = this.createMessage({ to, subject, body: formattedBody, from, CRLF });
            ...
            const response = await this.gmail.users.messages.send({
                ...
                requestBody: {
                    raw: encodedMessage,
                    threadId, // 👈 This keeps the email in the same thread
                },
            });
            ...
            console.error('Gmail sendEmailInThread error:', error);
